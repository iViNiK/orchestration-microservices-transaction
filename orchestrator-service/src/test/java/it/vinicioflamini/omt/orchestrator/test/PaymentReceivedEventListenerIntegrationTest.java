package it.vinicioflamini.omt.orchestrator.test;
...
import it.vinicioflamini.omt.orchestrator.listener.PaymentReceivedEventListener;
import it.vinicioflamini.omt.orchestrator.rest.ShipmentRestClient;

@RunWith(SpringRunner.class)
@SpringBootTest(classes = { PaymentReceivedEventListenerIntegrationTest.App.class, TestProducer.class, PaymentReceivedEventListener.class })
@EnableBinding(OrchestratorChannel.class)
public class PaymentReceivedEventListenerIntegrationTest {
	...
	@Autowired
	private PaymentReceivedEventListener consumer;
	...
	@Test
	public void testPaymentReceivedThenDoProcessShipment() {
		OrderEvent event = new OrderEvent();
		event.setOrderId(10L);
		event.setAction(Action.PAYMENTRECEIVED);
		...
		assertEquals(Action.PAYMENTRECEIVED, consumer.getReceivedMessage().getAction());
